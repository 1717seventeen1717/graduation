...
//					$(this).parent().find('p').eq(0).html(`<em></em>工人口令不正确`).css('display', 'block');
    ...
    //验证地区按钮
    $('#areaIp').blur(function() {
            if (this.value != '') {
				bstoparea = false;
                $('#areaIp').parent().find('.i-status').css('display', 'block');
                $(this).parent().find('p').eq(0).css('display', 'none');
            } else {
                $('#areaIp').parent().find('p').eq(0).html(`<em></em>地址不能为空`).css('display', 'block');
                $('#area .warning-title').value='';
                bstoparea = true;
            }
            if (bstoparea) {
                $('#areaIp').parent().find('.i-status').css('display', 'none');
                $('#area').css('border','1px solid red');
                $('#areaIp').parent().find('p').css('color', 'red');
                $('#areaIp').parent().find('em').css('background-position', '-17px -100px');
            }
            else{
            	$('#area').css('border', '1px solid #ccc');
            	$('#areaIp').parent().find('p').css('color', '#ccc');
            	$('#areaIp').parent().find('.i-status').css('display', 'block');
            }
        });
